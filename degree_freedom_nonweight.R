...
deg.frdm = function(X, y, beta.coef, a = 0, lambda, type = c("lasso", "bridge", "scad", "mcp")){
  ...
  n = length(y)
    ...
    if(type == "bridge"){
      ...
      deg_frdm = sum(diag(x.beta%*%solve((t(x.beta)%*%x.beta + a*sig.lam), t(x.beta))))
    }
    if(type == "scad"){
      p.lam = matrix(0, length(ind))
      for(i in 1:length(beta.nz)){
      ...
      deg_frdm = sum(diag(x.beta%*%solve((t(x.beta)%*%x.beta + n*sig.lam), t(x.beta))))
    }
    if(type == "mcp"){
      p.lam = matrix(0, length(ind))
      for(i in 1:length(beta.nz)){
